...
static void handle_param_executeopcode(CanardInstance* ins, CanardRxTransfer* transfer)
{
    ...
    if (req.opcode == UAVCAN_PROTOCOL_PARAM_EXECUTEOPCODE_REQUEST_OPCODE_ERASE) {
        ...
        AP_Param::setup_object_defaults(&periph.battery, periph.battery.var_info);
#endif
...
void AP_Periph_FW::can_battery_update(void)
{
    ...
    if (now_ms - battery_last_can_send_ms < 100) {
        return;
    }
    battery_last_can_send_ms = now_ms;

    const uint8_t battery_instances = battery.num_instances();
    for (uint8_t i=0; i<battery_instances; i++) {
        ...
        const int32_t serial_number = battery.get_serial_number(i);
        pkt.battery_id = (serial_number >= 0) ? serial_number : i+1;

        pkt.voltage = battery.voltage(i);
        ...
        float temperature;
        if (battery.get_temperature(temperature, i)) {
